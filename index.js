...
client.on("ready", () => {
  ...
  client.user.setGame(
    client.guilds.size + " servers | " + config.prefix + "help"
  );
});
client.on("message", msg => {
  if (msg.content === "test") {
    ...
    msg.channel.sendFile(
      "https://media.giphy.com/media/8GY3UiUjwKwhO/giphy.gif"
    );
  }
  ...
  switch (args[0].toLowerCase()) {
    ...
    case "ping":
      ...
      var pingembed = new Discord.RichEmbed()
        ...
        .setTitle("Ping Usage")
        .setDescription(
          config.prefix + "ping text or " + config.prefix + "ping image"
        )
        .addField(
          config.prefix + "ping <image|text>",
          "<image|text> = Test your ping with an image or with text"
        );

      let pingmessage = message.content
        .split(" ")
        .slice(1)
        .join(" ");
      if (pingmessage.length < 1)
        return message.channel.send({ embed: pingembed });
      if (pingmessage === "text")
      ...
      if (pingmessage === "image")
        return message.channel.send("Pinging...").then(sent => {
          sent
            .edit(
              "http://madeformakers.org/wp-content/uploads/2016/01/pong.png"
            )
            .then(msg => {
              msg.edit(
    ...
    case "imageping":
      message.channel.send("Pinging...").then(sent => {
        sent
          .edit("http://madeformakers.org/wp-content/uploads/2016/01/pong.png")
          .then(msg => {
            msg.edit(
              `Pong! Took ${sent.createdTimestamp - message.createdTimestamp}ms`
            );
            console.log(
              "Image Ping performed on " +
                message.guild.name +
    ...
    case "play":
      var config = require("./config.json");
      let musictoplay = message.content
        .split(" ")
        .slice(1)
        .join("");
      let volume2set = message.content
        .split(/\s+/g)
        .slice(2)
        .join(" ");
      var musiclengtherror = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "play <youtube_link>",
          "<youtube_link> = Link to Youtube Video"
        );

      if (musictoplay.length < 1)
        return message.channel
          .send({ embed: musiclengtherror })
          .catch(console.error);
    ...
    case "queue":
      message.channel.send(
        "`Sorry, but I do not support queues at this time. Please invite HackerBot Music for those commands`"
      );

      break;
    case "skip":
      message.channel.send(
        "`Sorry, but I do not support skipping at this time. Please invite HackerBot Music for those commands`"
      );
    ...
    case "setusernick":
      ...
      var usernickerrorembed = new Discord.RichEmbed()
        ...
        .setDescription(
          "Please use " +
            config.prefix +
            "setnick | This command also works for bots."
        )
        .addField(
          config.prefix + "setnick <@user> <nick>",
          "<@user> = User to nick for | <nick> = Nickname"
        );
    ...
    case "setbotnick":
      ...
      break;
    case "setnick":
      let unickstaffRoleIDs = message.guild.roles
        .filter(
          r =>
            r.name == "Owner" ||
            r.name == "Co-Owner" ||
      ...
      if (unickisStaff) {
        ...
        var nickembed = new Discord.RichEmbed()
          ...
          .setColor("00FF00")
          .setDescription(
            "You must provide a nickname and a user *or bot* to set a nickname for."
          )
          .addField(
            config.prefix + "setnick <@user> <nick>",
            "<@user> = Mentioned user | <nick> = Nickname"
          );

        let nickuserset = message.guild.member(message.mentions.users.first());
        let usernick = message.content
          .split(/\s+/g)
          .slice(2)
          .join(" ");
        if (nickuserset < 1)
          return message.channel
            .send({ embed: nickembed })
            .catch(console.error);
        if (usernick < 1)
          return message.channel
            .send({ embed: nickembed })
        ...
        message.channel.send(
          "Check " +
            nickuserset +
            "'s nick/username to see if the results match " +
            usernick
        );
      } else {
    ...
    case "dmuser":
      ...
      var dmuembed = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "dmuser <@user> <message>",
          "<@user> = Mentioned User | <message> = message to send."
        );

      let message2send2user = message.content
        .split(/\s+/g)
        .slice(2)
        .join(" ");
      let user = message.guild.member(message.mentions.users.first());
      if (message2send2user.length < 1)
        return message.channel.send({ embed: dmuembed }).catch(console.error);
      if (user.length < 1)
        return message.channel.send({ embed: dmuembed }).catch(console.error);
      message.guild
        .member(user)
    ...
    case "setgame":
      var gameembed = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "setgame <game>",
          "<game> = Can be anything really"
        );

      let playgame = message.content
        .split(" ")
        .slice(1)
        .join(" ");
      if (playgame.length < 1)
        return message.channel.send({ embed: gameembed }).catch(console.error);
    ...
    case "consoleissue":
      var ciembed = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "consoleissue <issuereport>",
          "<issuereport> = Your issue"
        );

      let issuereport = message.content
        .split(" ")
        .slice(1)
        .join(" ");
      if (issuereport.length < 1)
        return message.channel.send({ embed: ciembed }).catch(console.issue);
      console.log(
        "Issue report: " + issuereport + " from " + message.guild.name + "."
      );
      message.delete();
    ...
    case "rage":
      message.channel.send(
        "https://giphy.com/gifs/glee-image-wiki-wvQIqJyNBOCjK"
      );
      break;
    case "uptime":
      ...
      let uptimeformat = message.content
        .split(" ")
        .slice(1)
        .join(" ");
      ...
      var dayupembed = new Discord.RichEmbed()
        ...
        .addField("Uptime Days", days);
      if (uptimeformat === "seconds")
        return message.channel.send({ embed: secondupembed });
      if (uptimeformat === "minutes")
        return message.channel.send({ embed: minuteupembed });
      if (uptimeformat === "hours")
        return message.channel.send({ embed: hourupembed });
      if (uptimeformat === "days")
        return message.channel.send({ embed: dayupembed });
      if (uptimeformat.length < 1)
    ...
    case "membercount":
      var mcembed = new Discord.RichEmbed()
        ...
        .addField(
          "Offline Member Count",
          message.guild.presences.filter(p => p.status == "offline").size
        );
    ...
    case "success":
      message.channel.sendFile(
        "https://cdn.discordapp.com/attachments/216032723851018251/346777021780590612/meme.png"
      );
    ...
    case "warn":
      ...
      var warnembed = new Discord.RichEmbed()
        ...
        .addField(
          ...
          "<@user> = @Mentioned User | <reason> = Reason for warn"
        );
      ...
      if (message.mentions.users.size < 1)
        return message.channel.send({ embed: warnembed }).catch(console.error);
    ...
    case "dmhacker":
      ...
      var dmhembed = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "dmhacker <DM>",
          "<DM> = Direct Message to send to TheHacker894"
        );
      let message4hacker = message.content
        .split(" ")
        .slice(1)
        .join(" ");
      if (message4hacker.length < 1)
        return message.channel.send({ embed: dmhembed }).catch(console.error);
      message.delete();
      client.users
        .get("270375857384587264")
        .send(
          "A message from the user " +
            message.author.toString() +
            " has arrived."
      ...
      break;
    case "ban":
      let staffRoleIDs = message.guild.roles
        .filter(
          r =>
            r.name == "Owner" ||
            r.name == "Co-Owner" ||
      ...
      if (isStaff) {
        ...
        var embedreturn = new Discord.RichEmbe()
          .setColor("00FF00")
          .setDescription(
            "You need to provide a member to ban and a reason for the ban"
          )
          .addField(
            config.prefix + "ban <@user> <reason>",
            "<@user> = Mentioned User | <reason> = Reason for Ban"
          );

        let banMember = message.guild.member(message.mentions.users.first());
        let banreason = message.content
          .split(/\s+/g)
          .slice(2)
          .join(" ");
        if (banreason.length < 1)
          return message.channel
            .send({ embed: embedreturn })
            .catch(console.error);
        if (banMember.length < 1)
          return message.channel
            .send({ embed: embedreturn })
            .catch(console.error);
        ...
        var embedpermreturn = new Discord.RichEmbed()
          ...
          .setTitle("Ban Usage")
          .setDescription(
            "This command will only work if your role is named Owner or Admin."
          )
          .addField(
            config.prefix + "ban <@user> <reason>",
            "<@user> = Mentioned User | <reason> = Reason for Kick"
          )
          .setFooter(
            "Version " + info.newversion + " (Ban Usage)",
            "https://cdn.discordapp.com/avatars/342054071060004884/04f1a7a65bd8467eb928e82b33e6d010.p?size=1024"
          );
    ...
    case "unixtime":
      message.channel.send(
        "The amount of ticks that have passed since 1/1/1970 on EDT is " +
          Date.now()
      );
      break;
    case "kick":
      let staffRoleID = message.guild.roles
        .filter(
          r =>
            r.name == "Owner" ||
            r.name == "Co-Owner" ||
      ...
      if (isStaffs) {
        ...
        var newembed = new Discord.RichEmbed()
          ...
          .setFooter(
            "Version " + info.newversion + " (Kick Usage)",
            "https://cdn.discordapp.com/avatars/342054071060004884/04f1a7a65bd8467eb928e82b33e6d010.webp?size=1024"
          );
        ...
        let reason = message.content
          ...
          .join(" ");
        if (reason.length < 1)
          return message.channel.send({ embed: newembed }).catch(console.error);
        if (kickMember.length < 1)
          return message.channel.send({ embed: newembed }).catch(console.error);
        ...
        var config = require("./config.json");
        {
          ...
          var embedreturn = new Discord.RichEmbed()
            ...
            .setTitle("Kick Usage")
            .setDescription(
              "This command will only work if your role is named Owner, Admin, or Moderator"
            )
            .addField(
              config.prefix + "kick <@user> <reason>",
              "<@user> = Mentioned User | <reason> = Reason for Kick"
    ...
    case "purge":
      let purgeids = message.guild.roles
        .filter(
          r =>
            r.name == "Owner" ||
            r.name == "Co-Owner" ||
      ...
      if (isStaffspurge) {
        ...
        var lengthtoosmall = new Discord.RichEmbed()
          .setColor("00FF00")
          .setDescription(
            "You must provide a number of message to purge; 1 - 100"
          )
          .addField(
            config.prefix + "purge <amount>",
            "<amount> = Messages to purge"
          );

        var lengthtoobig = new Discord.RichEmbed()
          .setColor("00FF00")
          .setDescription(
            "The amount of messages to purge cannot be greater than 100"
          )
          .addField(
            config.prefix + "purge <amount>",
            "<amount> = Messages to purge (cannot be greater than 100)"
          );

        let purgearg = message.content
          .split(" ")
          .slice(1)
          .join(" ");
        if (purgearg.length < 1)
          return message.channel
            .send({ embed: lengthtoosmall })
            .catch(console.error);
        if (purgearg.length > 100)
          return message.channel
            .send({ embed: lengthtoobig })
            .catch(console.error);
        message.delete();
        ...
        var permembed = new Discord.RichEmbed()
          .setColor("00FF00")
          .setDescription(
            "Your role must be named Owner, Admin, or Co-Owner for this command"
          )
          .addField(
            config.prefix + "purge <amount>",
            "<amount> = Messages to purge"
    ...
    case "channeldelete":
      let staffchanneldelID = message.guild.roles
        .filter(
          r =>
            r.name == "Owner" ||
            r.name == "Co-Owner" ||
      ...
      if (isStaffchanneldel) {
        ...
        console.log(
          "A channel has been deleted on " + message.guild.name + "."
        );
      } else {
        return message.reply(
          " You don't have the perms to delete channels you fake hacker"
        );
      }
      break;
    case "channelcreate":
      let staffchannelcreateID = message.guild.roles
        .filter(
          r =>
            r.name == "Owner" ||
            r.name == "Co-Owner" ||
      ...
      if (isStaffchannelcreate) {
        ...
        var channelcembed = new Discord.RichEmbed()
          ...
          .addField(
            config.prefix + "channelcreate <channelname>",
        ...
        if (channelname.length < 1)
          return message.channel.send({ embed: channelcembed });
        ...
        console.log(
          "A channel has been created on " + message.guild.name + "."
        );
      } else {
        var config = require("./config.json");
    ...
    case "noticeme":
      message.channel.send(
        "You have been noticed, " + message.author.toString() + "."
      );
      break;
    case "noticemesenpai":
      message.channel.send(
    ...
    case "sourcecode":
      var scembed = new Discord.RichEmbed()
        ...
        .setDescription(
          "Here you can view and download the source code. Please note that the GitHub Source Code gets updated every week."
        )
    ...
    case "invite":
      message.channel.send(
        "Here is the invite to the server requested by," +
          message.author.toString() +
          "."
      );
    ...
    case "botinvite":
      message.channel.send(
        "Here is an invite for HackerBot requested by, " +
          message.author.toString() +
          "."
      );
      var binvembed = new Discord.RichEmbed()
        ...
        .setURL(
          "https://discordapp.com/oauth2/authorize?client_id=342054071060004884&scope=bot&permissions=2146958591"
        );
    ...
    case "info":
      ...
      var infoembed = new Discord.RichEmbed()
        ...
        .addField(
          "Helpers",
          "@Google Drive#0831, @Bloxxer_DTC#1958, The Discord.js Discord Server",
          false
        )
        .addField(
          "Testers",
          "@Corbs#9620, Ion#8844, @Google Drive#0831, @Shadow The  |「Dimensions™」#5869, @Jackalope#6413",
          false
        )
        .addField("Version", info.newversion)
        .setThumbnail(
          "https://images-ext-1.discordapp.net/external/Z1krf1ZBQTM3mDkke47xThwIF4GEFG0doRAdKRVHgnY/%3Fsize%3D128/https/cdn.discordapp.com/avatars/342054071060004884/4b83b2aa5970b8d34db71fd986abfe30.jpg"
        )
        .setFooter(
      ...
      break;
    case "help":
      let helpcommand = message.content
        .split(" ")
        .slice(1)
        .join(" ");
      var info = require("./info.json");
      ...
      var helpembed = new Discord.RichEmbed()
        ...
        .setDescription(
          config.prefix +
            "help <command> to get help with all commands listed here with the exclusion of the **WIP** commands."
        )
        .addField(
          "**Information**",
          "`help` `ping` `test` `info` `uptime` `serverinfo`"
        )
        .addField(
          "**Bot Information**",
          "`botinvite` `website` `musicwebsite` `sourcecode` `github` `suggestion` `consoleissue`"
        )
        .addField("**Music**", "`play` `skip` `pause` `queue` `end` `stop`")
        .addField(
          "**Entertainment**",
          "`say` `saytts` `8ball` `coinflip` `flip` `roll`"
        )
        .addField(
          "**Memes**",
          "`rage` `error` `tableflip` `untableflip` `shrug` `notproductive` `bigorder`"
        )
        .addField(
          "**Moderation**",
          "`ban` `unban` `kick` `purge` `channelcreate` `channeldelete` `setnick`"
        )
        .addField(
          "**Other**",
          "`dmhacker` `dmuser` `devpage` `invite` `unixtime`"
        )
      ...
      var uptimeembed = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "uptime <timeformat>",
          "<timeformat> = Seconds, Minutes, etc,..."
        );
      ...
      var consoleissueembed = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "consoleissue <issuereport>",
          "<issuereport> = Issue to report to the console"
        );
      ...
      var sayttsembed = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "saytts <ttsmessage>",
          "<ttsmessage> = TTS Message to say"
        );
      var eightballembed = new Discord.RichEmbed()
        .setColor("00FF00")
        .setTitle("8Ball Help")
        .setDescription(
          "Ask the mystical 8Ball your :fire: burnining :fire: questions"
        )
        .addField(
          config.prefix + "8ball <question>",
          "<question> = Question to ask the mystical 8Ball"
        );
      ...
      var errorccembed = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "error <color>",
          "<color> = Any color from the rainbow"
        );
      ...
      var channelcreateembed = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "channelcreate <name>",
          "<name> = Name for Channel Creation"
        );
      ...
      var setnickembed = new Discord.RichEmbed()
        ...
        .setDescription(
          "Set the Nickname of a user or bot *(with the correct permissions)*"
        )
        .addField(
          config.prefix + "setnick <@user> <nickname>",
          "<@user> = @Mentioned User ! <nickname> = Nickname to set for the user or bot"
        );
      var dmuserembed = new Discord.RichEmbed()
        .setColor("00FF00")
        .setTitle("DMUser Help")
        .setDescription(
          "DM a user through HackerBot; **This command does not work globally**"
        )
        .addField(
          config.prefix + "dmuser <@user> <message>",
          "<@user> = @Mentioned User | <message> = Message to send to user"
        );
      var dmhackerembed = new Discord.RichEmbed()
        ...
        .addField(
          config.prefix + "dmhacker <message>",
          "<message> = Message to send to TheHacker894"
        );
      var devpageembed = new Discord.RichEmbed()
        .setColor("00FF00")
      ...
      var inviteembed = new Discord.RichEmbed()
        ...
        .setDescription(
          "Sends an Invite Link to the HackerWorld Discord Server"
        )
      ...
      var unixtimeembed = new Discord.RichEmbed()
        ...
        .setDescription(
          "Sends the amount of ticks passed since 1/1/1970 on EDT"
        )
      ...
      if (helpcommand === `help`)
        return message.channel.send({ embed: helpcembed });
      if (helpcommand === `ping`)
        return message.channel.send({ embed: pingembed });
      if (helpcommand === `test`)
      ...
      if (helpcommand === `botinvite`)
        return message.channel.send({ embed: botinviteembed });
      if (helpcommand === `website`)
        return message.channel.send({ embed: websiteembed });
      if (helpcommand === `musicwebsite`)
        return message.channel.send({ embed: musicwebsiteembed });
      if (helpcommand === `sourcecode`)
        return message.channel.send({ embed: sourcecodeembed });
      if (helpcommand === `github`)
        return message.channel.send({ embed: githubembed });
      if (helpcommand === `suggestion`)
        return message.channel.send({ embed: suggestionembed });
      if (helpcommand === `consoleissue`)
        return message.channel.send({ embed: consoleissueembed });
      if (helpcommand === `play`)
        return message.channel.send(
          "You must invite HackerBot Music for this command - https://discordapp.com/oauth2/authorize?client_id=365255372480446465&scope=bot&permissions=2146958591"
        );
      if (helpcommand === `pause`)
        return message.channel.send(
          "You must invite HackerBot Music for this command - https://discordapp.com/oauth2/authorize?client_id=365255372480446465&scope=bot&permissions=2146958591"
        );
      ...
      if (helpcommand === `say`)
        return message.channel.send({ embed: sayembed });
      if (helpcommand === `saytts`)
        return message.channel.send({ embed: sayttsembed });
      if (helpcommand === `8ball`)
        return message.channel.send({ embed: eightballembed });
      if (helpcommand === `coinflip`)
        return message.channel.send({ embed: coinflipembed });
      if (helpcommand === `flip`)
        return message.channel.send({ embed: coinflipembed });
      if (helpcommand === `roll`)
        return message.channel.send({ embed: rollembed });
      if (helpcommand === `rolldice`)
        return message.channel.send({ embed: rollembed });
      if (helpcommand === `rage`)
        return message.channel.send({ embed: rageembed });
      if (helpcommand === `error`)
        return message.channel.send({ embed: errorccembed });
      if (helpcommand === `tablelfip`)
        return message.channel.send({ embed: tableflipembed });
      if (helpcommand === `untableflip`)
        return message.channel.send({ embed: untableflipembed });
      if (helpcommand === `shrug`)
        return message.channel.send({ embed: shrugembed });
      if (helpcommand === `notproductive`)
        return message.channel.send({ embed: notproductiveembed });
      if (helpcommand === `bigorder`)
        return message.channel.send({ embed: bigorderembed });
      if (helpcommand === `ban`)
        return message.channel.send({ embed: banembed });
      if (helpcommand === `unban`)
        return message.channel.send({ embed: unbanembed });
      if (helpcommand === `purge`)
        return message.channel.send({ embed: purgeembed });
      if (helpcommand === `kick`)
        return message.channel.send({ embed: kickembed });
      if (helpcommand === `channelcreate`)
        return message.channel.send({ embed: channelcreateembed });
      if (helpcommand === `channeldelete`)
        return message.channel.send({ embed: channeldeleteembed });
      if (helpcommand === `setnick`)
        return message.channel.send({ embed: setnickembed });
      if (helpcommand === `dmuser`)
        return message.channel.send({ embed: dmuserembed });
      if (helpcommand === `dmhacker`)
        return message.channel.send({ embed: dmhackerembed });
      if (helpcommand === `devpage`)
        return message.channel.send({ embed: devpageembed });
      if (helpcommand === `invite`)
        return message.channel.send({ embed: inviteembed });
      if (helpcommand === `unixtime`)
        return message.channel.send({ embed: unixtimeembed });
      break;
    case "unban":
      let unbanstaffRoleIDs = message.guild.roles
        .filter(
          r =>
            r.name == "Owner" ||
            r.name == "Co-Owner" ||
      ...
      if (unbanisStaff) {
        ...
        var embedreturn = new Discord.RichEmbed()
          ...
          .addField(
            config.prefix + "unban <userid> <reason>",
            "<userid> = User's Guild ID | <reason> = Reason for Unban"
          );

        let unbanMember = message.mentions.users.first();
        let unbanreason = message.content
          .split(/\s+/g)
          .slice(1, 2)
          .join(" ");
        if (reason.length < 1)
          return message.channel.sendEmbed(embedreturn).catch(console.error);
        message.guild.unban(unbanreason);
        message.delete();
        message.channel.sendMessage(
          "The user, " + message.author.toString() + "has unbanned a member."
        );
        ...
        var embedpermerror = new Discord.RichEmbed()
          ...
          .setTitle("Unban Usage")
          .setDescription(
            "This command will only work if you have a role named Owner or Admin."
          )
          .addField(
            config.prefix + "unban <userid> <reason>",
            "<userid> = User's Guild ID | <reason> = Reason for Unban"
    ...
    case "shrug":
      message.channel.sendFile(
        "https://media.giphy.com/media/ZeNmLY6FISq4M/giphy.gif"
      );
      break;
    case "notproductive":
      message.channel.send(
        "https://tenor.com/view/bouncing-weeeeee-gif-3451476"
      );
      break;
    case "suggestion":
      message.channel.send(
        "If you would like to submit a suggestion or concern visit one, or both, of the links below."
      );
    ...
    case "8ball":
      {
        ...
        if (question.length < 1)
          return message.channel.sendEmbed(questionembed).catch(console.error);
        console.log(
          message.guild.name +
            " | " +
            Date.now() +
    ...
    case "say":
      ...
      let message2say = message.content
        .split(" ")
        .slice(1)
        .join(" ");
      if (message2say.length < 1)
        return message.channel.send({ embed: sayembed }).catch(console.error);
    ...
    case "saytts":
      ...
      let message2saytts = message.content
        .split(" ")
        .slice(1)
        .join(" ");
      if (message2saytts < 1)
        return message.channel
          .send({ embed: sayttsembed })
          .catch(console.error);
    ...
    case "bigorder":
      message.channel.send(
        "I’ll have two number 9's, a number 9 large, a number 6 with extra dip, a number 7, two number 4's, one with cheese, and a large soda."
      );
      message.channel.sendFile(
        "http://pre06.deviantart.net/62e6/th/pre/f/2017/010/3/9/theorder_by_lawlsomedude-dauzlib.png"
      );
      break;
    case "untableflip":
    ...
    case "tableflip":
      message.channel.send("(╯°□°）╯︵ ┻━┻ ");
      message.channel.sendFile(
        "https://media.giphy.com/media/uKT0MWezNGewE/giphy.gif"
      );
      break;
    case "dannyslife":
      ...
      var dlembed = new Discord.RichEmbed()
        ...
        .setURL(
          "https://drive.google.com/file/d/0B3AjZwDXKX8fY0RUVTZUV0RrNGs/view?usp=sharing"
        );
      message.channel.send({ embed: dlembed });
      break;
    case "pause":
      message.channel.send(
        "`Sorry, but I do not support this feature at the moment, but you can invite HackerBot Music`"
      );
      message.channel.send(
        "https://discordapp.com/oauth2/authorize?client_id=365255372480446465&scope=bot&permissions=2146958591"
      );
      break;
    case "rps":
      var config = require("./config.json");
      ...
      var rpsitemlengtherrorembed = new Discord.RichEmbed()
        ...
        .addField(
          ...
          "<rock|paper|scissors> = Rock, Paper, or Scissors"
        );

      let rpsitem = message.content
        .split(" ")
        .slice(1)
        .join(" ");
      if (rpsitem === "rock")
      ...
      if (rpsitem === "scissor")
        return message.channel
          ...
          .catch(console.error);
      if (rpsitem === "gun")
        return message.channel.send(">:(").then(sent => {
          sent.edit(message.author.toString() + " :gun:");
        });
      if (rpsitem.length < 1)
        return message.channel.send({ embed: rpsitemlengtherrorembed });
      break;
    //   default:
...
client.login("NzQzNzY3NTIxMzUzNTMxNDU0.XzZdew.Jba7_Zn5ZivqlfBweZYe14_JdJg");
